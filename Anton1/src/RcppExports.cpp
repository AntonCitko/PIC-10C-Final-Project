// Generated by using Rcpp::compileAttributes() -> do not edit by hand
// Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

#include <Rcpp.h>

using namespace Rcpp;

// rcpp_hello_world
List rcpp_hello_world();
RcppExport SEXP _Anton1_rcpp_hello_world() {
BEGIN_RCPP
    Rcpp::RObject rcpp_result_gen;
    Rcpp::RNGScope rcpp_rngScope_gen;
    rcpp_result_gen = Rcpp::wrap(rcpp_hello_world());
    return rcpp_result_gen;
END_RCPP
}
// meanC
double meanC(NumericVector x);
RcppExport SEXP _Anton1_meanC(SEXP xSEXP) {
BEGIN_RCPP
    Rcpp::RObject rcpp_result_gen;
    Rcpp::RNGScope rcpp_rngScope_gen;
    Rcpp::traits::input_parameter< NumericVector >::type x(xSEXP);
    rcpp_result_gen = Rcpp::wrap(meanC(x));
    return rcpp_result_gen;
END_RCPP
}
// sumCpp
int sumCpp(IntegerVector x);
RcppExport SEXP _Anton1_sumCpp(SEXP xSEXP) {
BEGIN_RCPP
    Rcpp::RObject rcpp_result_gen;
    Rcpp::RNGScope rcpp_rngScope_gen;
    Rcpp::traits::input_parameter< IntegerVector >::type x(xSEXP);
    rcpp_result_gen = Rcpp::wrap(sumCpp(x));
    return rcpp_result_gen;
END_RCPP
}
// to_celsius
double to_celsius(double x);
RcppExport SEXP _Anton1_to_celsius(SEXP xSEXP) {
BEGIN_RCPP
    Rcpp::RObject rcpp_result_gen;
    Rcpp::RNGScope rcpp_rngScope_gen;
    Rcpp::traits::input_parameter< double >::type x(xSEXP);
    rcpp_result_gen = Rcpp::wrap(to_celsius(x));
    return rcpp_result_gen;
END_RCPP
}
// paste2
CharacterVector paste2(CharacterVector lhs, CharacterVector rhs);
RcppExport SEXP _Anton1_paste2(SEXP lhsSEXP, SEXP rhsSEXP) {
BEGIN_RCPP
    Rcpp::RObject rcpp_result_gen;
    Rcpp::RNGScope rcpp_rngScope_gen;
    Rcpp::traits::input_parameter< CharacterVector >::type lhs(lhsSEXP);
    Rcpp::traits::input_parameter< CharacterVector >::type rhs(rhsSEXP);
    rcpp_result_gen = Rcpp::wrap(paste2(lhs, rhs));
    return rcpp_result_gen;
END_RCPP
}
// removePunct
CharacterVector removePunct(const CharacterVector& ori);
RcppExport SEXP _Anton1_removePunct(SEXP oriSEXP) {
BEGIN_RCPP
    Rcpp::RObject rcpp_result_gen;
    Rcpp::RNGScope rcpp_rngScope_gen;
    Rcpp::traits::input_parameter< const CharacterVector& >::type ori(oriSEXP);
    rcpp_result_gen = Rcpp::wrap(removePunct(ori));
    return rcpp_result_gen;
END_RCPP
}
// removePunct2
CharacterVector removePunct2(CharacterVector ori);
RcppExport SEXP _Anton1_removePunct2(SEXP oriSEXP) {
BEGIN_RCPP
    Rcpp::RObject rcpp_result_gen;
    Rcpp::RNGScope rcpp_rngScope_gen;
    Rcpp::traits::input_parameter< CharacterVector >::type ori(oriSEXP);
    rcpp_result_gen = Rcpp::wrap(removePunct2(ori));
    return rcpp_result_gen;
END_RCPP
}
